...
function StarIcon({ filled, borderColor, size = 32 }: { filled: boolean; borderColor: string; size?: number }) {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      <Path
        d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"
        fill={filled ? '#F59E0B' : 'none'}
        stroke={filled ? '#F59E0B' : borderColor}
        strokeWidth={1.5}
...
export function RatingModal({ visible, onClose }: Props) {
  ...
  const handleDismiss = async () => { await markRatingShown(); onClose(); };
              ...
              <View style={s.starsRow}>
                {[1, 2, 3, 4, 5].map(star => (
                  <TouchableOpacity key={star} onPress={() => setRating(star)} activeOpacity={0.7}
                    ...
                    <StarIcon filled={star <= rating} borderColor={colors.border} size={36} />
                  </TouchableOpacity>
                ))}
              </View>
              {rating > 0 && rating < 4 && (
                <TextInput style={s.feedbackInput} placeholder="Tell us how we can improve..."
                  placeholderTextColor={colors.textQuaternary} multiline numberOfLines={3}
                  maxLength={500} value={feedback} onChangeText={setFeedback} textAlignVertical="top" />
              )}
              {rating > 0 && (
                <TouchableOpacity style={s.submitBtn} onPress={handleSubmit} activeOpacity={0.8}>
                  <Text style={s.submitText}>{rating >= 4 ? 'Rate on Play Store' : 'Submit Feedback'}</Text>
                </TouchableOpacity>
              )}
...
const makeStyles = (c: ColorPalette) => StyleSheet.create({
  ...
  card: {
    width: '85%', maxWidth: 340, backgroundColor: c.bg, borderRadius: RADIUS.lg,
  ...
  subtitle: { fontSize: 14, color: c.textTertiary, marginTop: 6, marginBottom: 24 },
  ...
  feedbackInput: {
    ...
    paddingHorizontal: 14, paddingVertical: 12, fontSize: 14, color: c.text, lineHeight: 20, marginBottom: 16,
  },
  submitBtn: { width: '100%', height: 48, borderRadius: RADIUS.md, backgroundColor: c.invertedBg, justifyContent: 'center', alignItems: 'center', marginBottom: 12 },
  submitText: { fontSize: 15, fontWeight: '600', color: c.invertedText },
  dismissBtn: { paddingVertical: 8 },
  dismissText: { fontSize: 14, color: c.textTertiary, fontWeight: '500' },
  thankYou: { alignItems: 'center', paddingVertical: 20 },
  thankTitle: { fontSize: 20, fontWeight: '700', color: c.text },
  thankSub: { fontSize: 14, color: c.textTertiary, marginTop: 8, textAlign: 'center' },
});
